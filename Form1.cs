        ...
        public static void DeleteOneOrder(int DtableNo, string DitemType, string DitemName, int Dquantity, bool kitchen)
        {
            ...
            if (item != null)
            {
                if (item.quantity > Dquantity)
                {
                    ...
                    if(!kitchen)
                    {
                        int totalcounter = chef.deleteinqueue(DitemName, DitemType, DtableNo, Dquantity);
                        int itempreparedcount = Dquantity - totalcounter;
                        if (itempreparedcount <= 0)
        ...
        public static void DeleteAllOrder(int DAtableNo, bool Cancel)
        {
            ...
            if (!Cancel)
            {
                for (int i = 0; i < orderList.Count(); i++)
                {
                    ...
                    {
                        orderTableAdapter.InsertItem(System.Convert.ToInt32(orderTableAdapter.GetCount()) + 1, orderList[i].tableNo, orderList[i].date);
                        break;
                    }
                }
            }
                ...
                if (!Cancel) // Add to report
                {
                    ...
                    for(int i=0; i<orderList.Count(); i++)
                    {
                        if (orderList[i].tableNo == DAtableNo)
                        {
                            ...
                            orderDetailTableAdapter.InsertItem(System.Convert.ToInt32(orderTableAdapter.GetCount()), tempID, orderList[i].quantity);
                            break;
            ...
            if (!Cancel && success)
                MessageBox.Show(String.Format("Table {0} has paid successfully", DAtableNo), "Success");
            if (!success)
